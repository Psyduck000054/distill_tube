...
function purgeVideos() { 
    showConfirmationModal("Are you sure? This deletes ALL videos.", () => { 
        fetch('/purge_videos', { method: 'POST' }).then(r => r.json()).then(d => { 
...
function deleteChannel(id, name) { 
    showConfirmationModal(`Unsubscribe from ${name}?`, () => { 
        fetch(`/delete_channel/${id}`, { method: 'POST' }).then(r=>r.json()).then(d=> { 
            if(d.success) { 
...
function moveVideo(id, act) { 
    ...
    if (card) {
        ...
        for (let b of badges) { if (b.innerText.trim() === 'New') { isFresh = true; break; } }
    }
