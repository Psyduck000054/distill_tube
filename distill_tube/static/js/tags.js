...
function addTag(t) { 
    ...
    tagState.tags.push(t); document.getElementById('tag-input-field').value=''; updateTagUI(); openDropdown(); }
function removeTag(t) { 
    tagState.tags = tagState.tags.filter(x => x !== t); 
    updateTagUI(); openDropdown(); 
    document.getElementById('tag-input-field').focus(); 
}
...
function openEditTagsModal(dbId, tags) {
  ...
  updateTagUI();
  document.getElementById('tag-picker-modal').classList.remove('hidden');
}
