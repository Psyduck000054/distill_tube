...
function openDropdown() {
  document
    .getElementById("suggested-tags-container")
...
function renderTagPills() {
  ...
  tagState.tags.forEach((t) => {
    ...
    p.className =
      "tag-pill text-xl font-bold px-5 py-2 flex items-center gap-3";
    p.setAttribute("data-tag", t);
  ...
  i.disabled = tagState.tags.length >= 3;
...
function renderSuggestedTags() {
  ...
  const v = document
    ...
    .value.trim()
    .toLowerCase();
  const s = allSystemTags.filter(
    (t) => !tagState.tags.includes(t) && t.toLowerCase().includes(v),
  );
  c.innerHTML = "";
  ...
  if (s.length === 0 && !v) {
    if (allSystemTags.length === 0)
      c.innerHTML =
        '<div class="p-6 text-gray-400 italic text-xl">No tags found.</div>';
    else
      c.innerHTML =
        '<div class="p-6 text-gray-400 italic text-xl">All selected.</div>';
    return;
  }

  if (s.length === 0 && v) {
    c.innerHTML =
      '<div class="p-6 text-gray-400 italic text-xl">No match. Press Enter to create.</div>';
    return;
  }
  ...
  s.forEach((t) => {
    ...
    b.className =
      "w-full text-left px-6 py-4 bg-white hover:bg-blue-50 border-b-2 border-gray-100 last:border-0 transition-colors block dark:bg-black dark:border-gray-800 dark:hover:bg-gray-900";
